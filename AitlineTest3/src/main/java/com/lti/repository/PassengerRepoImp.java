...
EntityManager em;
	...
	public List<Passenger> viewPassenger(int userId, int flightId,LocalDate dateOfTravel) {
		// TODO Auto-generated method stub
		TypedQuery<Passenger> passenger=em.createQuery("select p from Passenger p where p.user.userId = :uId and p.flight.flightNo = :fId "
	...
	/*public boolean findFlightInPassenger(int flightId) {
		// TODO Auto-generated method stub
		return (Long) em.createQuery("select count(p.flight.flightId) from Passenger p where p.flight.flightId = :fId")
